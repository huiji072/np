...
public class GetServerSocketFields {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(theServer.getInetAddress().getHostName() + " 호스트 및" + theServer.getLocalPort() +
					" 포트에 바인드된 서버 소켓을 생성하였다.");
		}catch(IOException e) {
			System.out.println(e);
			System.exit(1);
		}
		while(true) {
			System.out.println("클라이언트의 접속요청을 기다립니다.");
			try {
				//클라이언트가 접속요청을 할 때까지 무한히 기다린다.
				theSocket = theServer.accept();
				System.out.println(theSocket.getInetAddress().getHostName() 
						+" 이름 및 " + theSocket.getPort() + " 포트의 클라이언트로부터 접속요청을 받았습니다.");
